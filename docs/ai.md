...
| tier_sampler | 期望终局档位效用最大（保底优先、优秀其次、1120k 封顶） | 对齐 80% 保底 / 50% 优秀目标 |

参数 `samples`（默认 24，上限 128）与 `rollouts`（默认 16，上限 64）防止计算量失控。

`greedy` / `sampler` / `tier_sampler` 的未知未来样本由“当前可见观察 + 样本编号”确定，
不读取真实游戏 RNG。同一局面反复决策会得到相同结果，各候选动作还会共享同一组未来样本，
因此并行调度不会改变基准结果，也不会因为某个动作偶然抽到更好的未来而获胜。

`tier_sampler` 会先仅按初始怪物阵容锁定攻略流派，不让当前候选用具反向改变流派。
开局目标用具出现时立即选择；目标缺失且 pet 仍有用具刷新时优先刷新，最多使用 pet2
提供的两次机会。药剂刷新则优先放在前四瓶，用于寻找该流派的核心／铺垫药剂。
