...
当前规则／内容版本为 `rules-v3` / `cards-v3`，随机版本为 `splitmix64-v5`；流程为不计回合的独立开局用具、八次药剂与三次方案。内容目录覆盖并启用全部 24 项手术用具、40 项药剂（含四种药剂箱），怪物目录包含四族共 30 种具名怪物。不支持的历史版本不会静默迁移。
...
- `base_cursor`：12 次基础选择的位置，0 为开局用具，1–8 为八次药剂，9–11 为最后三次方案，12 表示基础选择完成。
- `completed_turns`：八次药剂、三次方案及 0–2 个分数奖励用具回合；不包含未知器具和开局用具选择。
...
stateDiagram-v2
    ...
    CHOOSING --> CHOOSING: 领取开局用具，展示首轮药剂，不计回合
    CHOOSING --> CHOOSING: 刷新或开箱，不计回合
    CHOOSING --> RESOLVING: 选择药剂、方案或分数奖励用具并提交目标
...
开局用具的 `reward_threshold=0`，仅推进一个基础流程位置，不增加正式回合、不触发回合结束效果或周期效果，也不检查用具分数解锁。分数用具的门槛为 8000/28000，每次增加一个正式回合并正常结算，但不推进基础流程。刷新保留门槛来源，不能把开局用具误当作分数奖励。未解锁的分数奖励不补发，因此正式回合共 11–13 次，最多为 2＋8＋3。用具跨批次允许重复出现和获得，按获得顺序逐件触发；同批用具候选不重复。药剂允许重复。
...
每次卡牌操作逐步执行，其直接触发的事件链处理完再执行下一步。用具按获得顺序执行，条件读取执行当时的场面。新获得的分数奖励用具参与本回合结束；开局用具从第一个正式回合起参与。正式回合数在回合结束触发前加一，用于周期效果，开局选择不能触发第零回合的周期效果。

融合和吞噬内部直接变更槽位，只发 `fused/devoured`，不发 `removed/added`。事件携带序号、父序号、来源、目标和属性变化，界面可解释连锁。每命令最多 512 个效果/事件执行步骤，超限整次回滚。

## 3. 属性、评分与演示细节

基础属性：普通 `1×36`，魔法 `5×24`，稀有 `15×12`，首领 `300×1`。变异/觉醒保留现值，再叠加最终目标稀有度的基础值；跨两阶只加最终目标基础值一次。按阶数觉醒的首领不再升级；指定稀有度的觉醒允许降阶和同阶。至纯圣水对原首领觉醒者随机抽取首领觉醒者，并增加 `300×1` 对应的活性／数量。融合分别求和，不另加生成怪物基础值；吞噬保留吞噬者身份。
...
- 普通药剂每个候选独立按 40/35/20/5 抽稀有度，再从该等级可选卡牌中抽取。为保持准确权重，样例池允许同一候选组出现同名药剂。用具同批候选无重复，但不排除已拥有用具；此抽取语义由 `splitmix64-v5` 标识，旧随机版本模拟器存档需重新开局，不影响 OCR 客户端观察记录。
...
- `monsters.go` 定义四族的具名怪物。生成时先抽种群与稀有度，再在同种群、同稀有度中等概率抽取具体怪物；名称和定义 ID 随状态、事件快照与观察一起保存。卵壳药粉添加同名怪物，黏稠胆汁溶液复制目标怪物身份。
- 疫区圣母像每次触发冻结各稀有度候选池，避免同一怪物连续跨池被再次觉醒。二度降生者之喙按 `活性×数量` 选择最低总活性目标，并排除吞噬者。
...
匿名 ID 首次由浏览器特征摘要加随机本地盐生成，后续复用已有 ID，不上传原始指纹。记录不支持跨域、跨浏览器同步。浏览器存储被清理或浏览器配额回收时可能丢失历史；无法依靠匿名指纹从服务端找回数据。配置 PostgreSQL 后，服务端另以 HMAC 假名记录局级种子和逐决策有限信息观察，用于 AI 训练；不保存原始匿名 ID、对局 ID、请求 ID、令牌、IP、User-Agent 或 RNG 游标，详见 `docs/training-data.md`。
